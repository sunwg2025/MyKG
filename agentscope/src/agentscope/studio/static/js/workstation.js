...
let nameToHtmlFile = {
    ...
    'KnowledgeAgent': 'agent-knowledgeagent.html',
...
async function addNodeToDrawFlow(name, pos_x, pos_y) {
    ...
    switch (name) {
       ...
       case 'KnowledgeAgent':
            const KnowledgeAgentID = editor.addNode('KnowledgeAgent', 1, 1, pos_x,
                pos_y, 'KnowledgeAgent', {
                    "args": {"name": 'ttt'}
                }, htmlSourceCode);
            var nodeElement = document.querySelector(`#node-${KnowledgeAgentID} .node-id`);
            if (nodeElement) {
                nodeElement.textContent = KnowledgeAgentID;
            }
            break;
