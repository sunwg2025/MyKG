...
if 'attribute_extract_results' not in st.session_state:
    st.session_state.attribute_extract_results = {}

if 'entity_extract_result' not in st.session_state:
    st.session_state.entity_extract_result = ''

if 'attribute_extract_prompt_reload' not in st.session_state:
    st.session_state.attribute_extract_prompt_reload = False

if 'attribute_extract_prompt_history' not in st.session_state:
    st.session_state.attribute_extract_prompt_history = ''

current_page = 'attribute_extract_page'
...
if 'current_experiment_id' not in st.session_state:
    ...
    with st.container(border=True):
        col1, col2 = st.columns([0.9, 0.1], vertical_alignment='bottom')
        with col1:
            if st.session_state.attribute_extract_prompt_reload:
                st.session_state['extract_prompt'] = prompt.attribute_extract
                st.session_state.attribute_extract_prompt_reload = False
            if st.session_state.attribute_extract_prompt_history != '':
                st.session_state['extract_prompt'] = st.session_state.attribute_extract_prompt_history
                st.session_state.attribute_extract_prompt_history = ''
            extract_prompt = st.text_area('提示词', height=360, max_chars=4096, key='extract_prompt')
        with col2:
            if st.button('载入默认'):
                st.session_state.attribute_extract_prompt_reload = True
                st.rerun()
            ...
            def logs_view():
                data = []
                experiment_logs = Experiment_Log.get_experiment_logs_by_experiment_id_and_type(
                    st.session_state.current_experiment_id, 'attribute_extract')
                for experiment_log in experiment_logs:
                    dataset = Dataset.get_dataset_by_id(experiment_log.dataset_id)
                    model = Model.get_model_by_id(experiment_log.model_id)
                ...
                if st.button('重载提示词'):
                    ...
                    if len(extract_prompts) > 1 and not error:
                        ...
                        error = True
                    if not error:
                        st.session_state.attribute_extract_prompt_history = extract_prompts[0]
                    st.rerun()

            if st.button('载入历史'):
                t = logs_view()

    st.subheader('Step 4：执行属性抽取', divider=True)
    if st.button('抽取知识'):
        st.session_state.attribute_extract_results = {}
        error = False
            ...
            with st.container(border=True):
                for model_select in models_select:
                    ...
                    extract_result = extract_attribute_knowledge(dataset_content=dataset_split.content,
                                                                 ...
                                                                 extract_parse=prompt.attribute_extract_parse)
                    result_key = '{}: {}'.format(model.id, model.name)
                    st.session_state.attribute_extract_results[result_key] = extract_result
    ...
    with st.form('submit'):
        for key in st.session_state.attribute_extract_results.keys():
            st.text_area('抽取结果：{}'.format(key), height=240, max_chars=4096,
                         value=st.session_state.attribute_extract_results[key], key=key)

        result_select = st.radio('选择抽取结果', options=st.session_state.attribute_extract_results.keys())
